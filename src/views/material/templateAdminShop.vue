  ...
  <div class="app-container">
    ...
    <div class="app-content">
      ...
      >
        ...
        <el-table-column property="dlabelShopTemplateTypeVO.name" label="模板分类" />
...
import { shoptemplatesystemInserts,shopupload_ddl,shoptemplatetypeSort,shoptemplateList, shoptemplatetypeAllList, shoptemplatetypeDelete, shoptemplateDelete, shoptemplatetypeInsert, shoptemplatetypeUpdate, shoptemplatetypeList } from '@/api/api'
...
export default {
  ...
  created() {
    // alert(this.text)
    this.listapi = shoptemplateList
    this.getListData(Object.assign({ pageNo: this.currentpage, pageSize: this.pagesize }, this.listQuery), shoptemplateList)
  ...
  methods: {
    ...
    editTypeDelete(id) {
      this.deleteDataType(id, shoptemplatetypeDelete, shoptemplatetypeList, this.listQuery)
    },
    ...
    handleDragEnd(e, item) {
      ...
      shoptemplatetypeSort({ json: JSON.stringify(tempData) })
    },
    ...
    addDataType() {
      ...
      this.addListDataType('ruleForm', shoptemplatetypeInsert, shoptemplatetypeUpdate, this.isedit, shoptemplatetypeList)
    },
    addDDLs(){
      shoptemplatesystemInserts({json:JSON.stringify(this.ddls),templateTypeId: this.listQuery.templateTypeId}).then(res=>{
        if(res.code==100){
    ...
    editBack(id) {
      this.$router.push({
        path: '/material/templateAdminEditShop',
        query: {
          id
        }
      })
    ...
    async getDlabelSysRoleList() {
      const res = await shoptemplatetypeAllList()
      if (res.code === 100) {
        res.data.unshift({
          id: '',
          name: '全部',
    ...
    editDelete(id) {
      this.deleteData(id, shoptemplateDelete, shoptemplateList, this.listQuery)
    }
  }
}
</script>
