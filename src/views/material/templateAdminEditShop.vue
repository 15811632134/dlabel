...
<script>
import {
  shoptemplatetypeAllList,
  shoptemplateInsert,
  shoptemplateUpdate,
  templatebackgroundList,
  templatebackgroundTotal,
  shoptemplatesystemSelect,
...
export default {
  ...
  methods: {
    ...
    async templatesystemSelect() {
      const res = await shoptemplatesystemSelect({
    ...
    async addTemplateList() {
      ...
      this.$refs.ruleForm.validate(async valid => {
        if (valid) {
          const url = this.$route.query.id ? shoptemplateUpdate : shoptemplateInsert
          ...
          if (res.code === 100) {
            ...
            $this.$router.push('/material/templateAdminShop')
          } else {
            this.$message({
              showClose: true,
              message: res.message,
