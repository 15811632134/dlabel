<template>
  <div class="userInfo-box">
    <div class="userInfo">
      ...
      <div class="userItem">
        <div class="header">业务信息<el-tooltip class="item" effect="dark" placement="right">
              <div slot="content" class="tooltip-content-box">
                以下信息是统计用户从注册时间到当前时间的总数据</div>
        ...
        <div class="itemDiv">
          <div class="m_clearLR">
            ...
            <div class="textContent m_floatL">{{ dataForm.total_print_area.toFixed(4) }}</div>
          </div>
        </div>
        <div class="itemDiv">
          <div class="m_clearLR">
        ...
        <div class="itemDiv">
          <div class="m_clearLR">
            ...
            <div class="textContent m_floatL">{{ dataForm.day_percent_print.toFixed(0) }}</div>
          </div>
        </div>
      </div>
      <div class="userItem">
        ...
        <div class="itemDiv">
          <div class="m_clearLR">
            <div class="m_floatL textHead">
              <img v-if="dataForm.sign_type" :src="require( `@/assets/img/${dataForm.sign_type}.png`)" style="width:19px;margin-right:17px" alt >
            </div>
    ...
    <div class="app-content">
      ...
      <div v-show="barIndex == 0&&dataForm.company_id==1" class="bar-tran-box" >
        <div :class="{active:currentIndex==0}" class="bar-tran-item" @click="selectData(0)">
          全部
        </div>
        <div :class="{active:currentIndex==1}" class="bar-tran-item" @click="selectData(1)">
          RFID耗材
        </div>
        <div :class="{active:currentIndex==2}" class="bar-tran-item" @click="selectData(2)">
          非RFID耗材
        </div>
      </div>
      <el-table
      ...
      >
        ...
        <!-- <el-table-column property="area" label="打印面积(m²)" sortable>
          <template slot-scope="{row}">
            <span>{{ row.area.toFixed(2) }}</span>
          </template>
        </el-table-column> -->
         <el-table-column
          width="180"
      ...
      >
        ...
        <el-table-column property="print_times" label="打印次数" sortable>
          <template slot-scope="{row}">
            <span>{{ row.print_times }}</span>
          </template>
        </el-table-column>
        <el-table-column property="print_count" label="打印数量" sortable>
          <template slot-scope="{row}">
            <span>{{ row.print_count }}</span>
          </template>
        </el-table-column>
      </el-table>

      <div class="pagination-container" v-show="totalnumber>0">
        <el-pagination
...
import { print_infoById, user_sign, paper_rfid, user_printer, paper_unrfid } from '@/api/api-python'
export default {
  filters: {
    formatDate(time) {
      if (time) {
  ...
  methods: {
    sortChange(val) {
      if (val.order == 'descending') { this.listQuery.ordering = '-' + val.prop } else { this.listQuery.ordering = val.prop }
    ...
    selectBarIndex(index) {
      ...
      if (index === 0) {
        ...
        if (this.listQuery.ordering.indexOf('area')!=-1) { this.listQuery.ordering = '' }
        user_printer({
          id: this.$route.query.id,
          page_size: this.listQuery.page_size,
          page: this.listQuery.page,
          company_id: this.listQuery.company_id,
          ordering: this.listQuery.ordering }).then(res => {
          this.loading = false
