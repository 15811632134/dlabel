<template>
  <div class="app-container">
    ...
    <div v-show="list.length>0" class="conserve">
      <el-button type="primary" class="m_search" @click="savePermission">返回</el-button>
    </div>
  </div>
</template>
...
import {
  ...
  get_company_role_autorty_by_role_id
} from '@/api/api'
export default {
  data() {
    return {
  ...
  created() {
    this.roleId = this.$route.query.t
    get_company_role_autorty_by_role_id({role_id:this.roleId}).then(response => {
      this.rolePermission = response.data.company_authority_ids
      ...
      var data = response.data.system_default_all_company_authority
  ...
  methods: {
    ...
    savePermission() {
      this.$router.push({name:'partnerRoleList'})
      // this.str = [];
