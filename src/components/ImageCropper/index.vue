...
import { updateFile } from '@/api/article'
export default {
  props: {
  ...
  methods: {
    ...
    createImg(e) {
      ...
      that.createImgUrl = canvas.toDataURL(this.fileData.type)
    },
    prepareUpload() {
      const {
        url,
  ...
  //return new Blob([u8arr],{type:mime});
    ...
    upload() {
      let that = this,
        ...
        fmData = new FormData()
        var img = this.tofile(createImgUrl, '1.'+this.fileData.type.split('/')[1])
        fmData.append(field, img)
        // fmData.append(field, data2blob(createImgUrl, 'image/jpg'), field + '.' + imgFormat)
        // 添加其他参数
      ...
      that.loading = 1
      that.setStep(3)
      updateFile(fmData).then(resData => {
        that.loading = 2
         this.$notify({
          title: "成功",
          message: "修改成功",
